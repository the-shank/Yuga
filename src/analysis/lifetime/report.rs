...
pub fn arg_return_uaf_report<'tcx>( tcx: &TyCtxt<'tcx>,
                                    func: &MirFunc<'tcx, '_>,
                                    inp_num: usize,
                                ...
                                ) -> (String, String)
{
    ...
    human_report.push_str("The latter can be longer than the former, which could lead to use-after-free!\n\n");
                            ...
                            ) -> (String, String)
{
    ...
    let src_name: String    = decompose_projection_as_str(&src_ty.projection, arg_name1.as_str().to_string());
    let tgt_name: String    = decompose_projection_as_str(&tgt_ty.projection, arg_name2.as_str().to_string());
    ...
    let trace = generate_trace(&src_ty, arg_name1.as_str().to_string(), get_actual_type(inp1, tcx).span, tcx);
    human_report.push_str(&trace);
    human_report.push_str("\n");

    let trace = generate_trace(&tgt_ty, arg_name2.as_str().to_string(), get_actual_type(inp2, tcx).span, tcx);
    human_report.push_str(&trace);
    human_report.push_str("\n");
