	            ...
	            if let Some(
		            ...
		            if let Some(def_id) = def_id {
		            	...
		            	if let Some(rustc_hir::Node::Item(
		                    rustc_hir::Item{vis_span: type_vis_span, ..}
		                )) = node
		                {
							type_vis =
				            	source_map
					...
					let impl_generic_bounds 	= get_bounds_from_generics(&this_impl.generics, &hir_map);
					...
					if let rustc_hir::TyKind::Path(rustc_hir::QPath::Resolved(_, path)) = this_impl.self_ty.kind {
